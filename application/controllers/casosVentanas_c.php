...
class CasosVentanas_c extends CI_Controller {
	...
	public function eliminarDerechoAfectado($derechoId, $idCaso){
		...
		if(!empty($datos['datosCaso']['derechoAfectado'][$derechoId])){
			foreach ($datos['datosCaso']['actos'] as $acto) {
				if ($acto['derechoAfectado_derechoAfectadoCasoId']==$derechoId) {
					if (isset($acto['victimas'])) {

						$data="No es posible eliminar este derecho afectado porque tiene victimas asociadas";
